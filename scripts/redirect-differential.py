...
"""
"""Differential harness: every generated redirect spelling, real bash vs the hook.

For each spelling the harness observes what bash actually did — the argv the
command received, and the file the redirect opened — and compares it against the
accesses scriptcheck derives. A demand bash's behaviour does not justify is an
over-approximation; a file bash touched with no demand is a bypass.
"""
...
COMMANDS = [
    ("cat", roles_cat, "cat in.txt {redir} zzz"),
    ("cp", roles_cp, "cp in.txt {redir} zzz"),
...
def cases():
    for name, roles, template in COMMANDS:
        for op, mode in OPS.items():
            for w in WORDS:
                yield name, roles, template, op, mode, w
...
def run_probe(d, op, w, template, seed):
    ...
    created = sorted(c for c in (after - before) if c != "argv.out")
    argv = []
...
def main():
    ...
    cmds = [t.replace("{redir}", op + w) for _, _, t, op, _, w in all_cases]
    ...
    def run_one(i):
        name, roles, template, op, mode, w = all_cases[i]
        cmd = cmds[i]
        ...
        if mode in ("write", "readwrite"):
            for c in created:
                expected.add(("Write", resolve(c)))
        if mode in ("read", "readwrite"):
            for m in set(missing) | set(seeded):
                expected.add(("Read", resolve(m)))
            if mode == "readwrite":
                for c in created:
                    expected.add(("Read", resolve(c)))
        actual = set()
        ...
        return dict(cmd=cmd, argv=argv, created=created, missing=missing, seeded=seeded,
                    ran=ran, stderr=stderr, expected=expected, actual=actual)

    with ThreadPoolExecutor(max_workers=16) as pool:
        results = list(pool.map(run_one, range(len(all_cases))))

    bypasses, extras, aborted = [], [], 0
    for r in results:
        ...
        if not r["ran"]:
            # bash abandoned the command, so it performed none of the accesses
            # the checker derives. Over-approximation is only meaningful where
            # the command ran; a missed access still counts.
            aborted += 1
            continue
        if extra:
            extras.append((r, extra))

    print(f"\n=== {len(results)} spellings, {len(bypasses)} missed accesses, {len(extras)} over-approximations, {aborted} that bash refused to run")
